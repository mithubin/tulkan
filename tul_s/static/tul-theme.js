...
const MODULE_PRESETS = {
  ...
  nach:  { chrome: '#141820', accent: '#8080c0' },  // Neutral-Lila  — Recherche
  'kal-trel': { chrome: '#0f1c1a', accent: '#3dbf8f' },  // Teal         — Planung
  buch:       { chrome: '#1a1510', accent: '#c09050' },  // Pergament    — Buch/Druck
  hub:        { chrome: '#1e1b18', accent: '#a07040' },  // Warm-Dunkel  — Zentrum
};
...
// ── State ─────────────────────────────────────────────────────────────────────
let _subpath = '', _onApply = null, _defaultPreset = null;
...
<div class="tlt-float-panel" id="tlt-panel-main" style="display:none">
  ...
  <div class="tlt-fp-body" id="tlt-body-main">
    ...
    <hr class="tlt-fp-sep">
    <div class="tlt-fp-section">Text</div>
    <div style="margin-bottom:4px;font-size:10px;color:#667">Schriftfarbe — Farbanteil des Textes</div>
    <div class="tlt-ctrl">
      <div class="tlt-ctrl-lbl">Textfarbe <span class="tlt-val" id="tlt-lbl-textcol">15%</span></div>
      <input type="range" id="tlt-m-textcol" min="0" max="60" value="15">
    </div>
    <hr class="tlt-fp-sep">
...
function computeTheme(p) {
  ...
  let v = {};
  // text_col (Hauptpanel-Slider) überschreibt text_s aus Lab wenn > 0
  const effectiveTextS = p.text_col > 0 ? p.text_col / 100 : p.text_s;
  if (isDark) {
    ...
    v['--text']   = hslToHex(h,    cs * effectiveTextS, p.text_l);
    ...
    v['--text']   = hslToHex(h,    cs * effectiveTextS, clamp(115 - p.text_l,  4, 30));
...
function update() {
  ...
  $('lbl-textcol').textContent = p.text_col > 0 ? p.text_col + '%' : 'Lab';
...
async function loadFromServer() {
  if (_subpath === null || _subpath === undefined) return;
  try {
    const r = await fetch(_subpath + '/theme');
    if (r.ok) {
      const saved = await r.json();
      restoreParams(saved);
      if (Array.isArray(saved._snaps) && saved._snaps.length) {
        _log = saved._snaps;
        renderLog();
      }
    }
...
async function saveToServer(params) {
  if (_subpath === null || _subpath === undefined) return;
  try {
    const computed = computeTheme(params);
    await fetch(_subpath + '/theme', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...params, ...computed, _snaps: _log }),
    });
  } catch (e) {}
}
...
function restoreParams(p) {
  ...
  $('l-blur').value    = p.blur;
  if (p.text_col !== undefined) $('m-textcol').value = p.text_col;
}
...
// ── Drag-Handler für Float-Panels ────────────────────────────────────────────
function makeDraggable(el, handle) {
  let _dragged = false;
  handle.style.cursor = 'grab';
  handle.addEventListener('pointerdown', e => {
    if (e.target.closest('.tlt-fp-toggle')) return;
    ...
    function onMove(ev) {
      ...
      if (!_dragged && Math.hypot(dx, dy) > 5) {
        _dragged = true;
      ...
      if (_dragged) {
        ...
        el.style.top  = clamp(oy + dy, 0, window.innerHeight - 60) + 'px';
      }
    }
    function onUp() {
      if (_dragged) handle.style.cursor = 'grab';
  ...
  return { wasDragged: () => { const v = _dragged; _dragged = false; return v; } };
}
...
function injectHelpUI() {
  ...
  style.textContent = `
    ...
    .tlt-help-btn:hover { border-color: var(--acc, #7c9cbf); color: var(--acc, #7c9cbf); }
    ...
    .tlt-help-overlay.tlt-open { display: flex; }
    ...
    .tlt-help-close {
      ...
      color: var(--muted, #778); font-size: 18px; cursor: pointer;
      line-height: 1; transition: color .15s;
    }
    .tlt-help-close:hover { color: var(--text, #dde); }

    .tlt-help-title {
      font-size: 16px; font-weight: 700;
      color: var(--acc, #7c9cbf);
      margin-bottom: 18px; padding-right: 24px;
    }
    .tlt-help-section { margin-bottom: 18px; }
    .tlt-help-section:last-child { margin-bottom: 0; }
    .tlt-help-heading {
      font-size: 11px; text-transform: uppercase;
      letter-spacing: .07em; color: var(--muted, #778);
    ...
    .tlt-help-steps li:last-child { border-bottom: none; }
    .tlt-step-n {
      ...
      color: var(--acc, #7c9cbf); flex-shrink: 0; margin-top: 2px;
    }
    .tlt-help-warn {
      margin-top: 6px; padding: 9px 12px;
      background: #1e1408; border: 1px solid #5a3a10;
  ...
  overlay.id = 'tlt-help-overlay';
  overlay.className = 'tlt-help-overlay';
  overlay.innerHTML = `
    <div class="tlt-help-modal">
      <button class="tlt-help-close" id="tlt-help-close">✕</button>
      <div id="tlt-help-content"></div>
    </div>
  `;
  document.body.appendChild(overlay);

  document.getElementById('tlt-help-close').addEventListener('click', _closeHelp);
...
function _openHelp() {
  const content = document.getElementById('tlt-help-content');
  const help = window.TUL_HELP;
  let html = '';

  if (!help) {
    html = `<div class="tlt-help-title">Anleitung</div>
      <p style="font-size:13px;color:var(--muted,#778)">Für dieses Tool ist noch keine Anleitung hinterlegt.</p>`;
  } else {
    html = `<div class="tlt-help-title">${help.title || 'Anleitung'}</div>`;
    (help.sections || []).forEach(sec => {
      html += `<div class="tlt-help-section">`;
      if (sec.heading) html += `<div class="tlt-help-heading">${sec.heading}</div>`;
      if (sec.text)    html += `<div class="tlt-help-text">${sec.text.replace(/\n/g, '<br>')}</div>`;
      if (sec.items) {
        html += `<ul class="tlt-help-list">`;
        sec.items.forEach(item => { html += `<li>${item}</li>`; });
        html += `</ul>`;
      }
      if (sec.steps) {
        html += `<ol class="tlt-help-steps">`;
        sec.steps.forEach((step, i) => {
          html += `<li><span class="tlt-step-n">${i + 1}</span><span>${step}</span></li>`;
        });
        html += `</ol>`;
      }
      if (sec.warn) html += `<div class="tlt-help-warn">${sec.warn}</div>`;
      html += `</div>`;
    });
  }

  content.innerHTML = html;
  document.getElementById('tlt-help-overlay').classList.add('tlt-open');
}

function _closeHelp() {
  const el = document.getElementById('tlt-help-overlay');
  if (el) el.classList.remove('tlt-open');
}

// ── Events ────────────────────────────────────────────────────────────────────
function setupEvents() {
  ['m-dcl','m-sweet','m-sat','m-textcol',
  ...
  const _mDrag = makeDraggable($('panel-main'), $('hdr-main'));
  const _lDrag = makeDraggable($('panel-lab'),  $('hdr-lab'));
  $('hdr-main').addEventListener('click', () => { if (!_mDrag.wasDragged()) togglePanel('main'); });
  $('hdr-lab').addEventListener('click',  () => { if (!_lDrag.wasDragged())  togglePanel('lab');  });
...
const tulTheme = {
  ...
  init(opts = {}) {
    ...
    // Server-Theme laden → überschreibt Default-Preset
    loadFromServer().then(() => update());

    // ── Header: rechte Gruppe (Theme + Hilfe + © + User) ────────────────────
    injectHelpUI();  // Help-Overlay DOM aufbauen
    const _hdr = document.querySelector('header');
    if (_hdr) {
      let rg = _hdr.querySelector('.tlt-hdr-right');
      if (!rg) {
        rg = document.createElement('div');
        rg.className = 'tlt-hdr-right';
        _hdr.appendChild(rg);
      }

      const themeBtn = document.createElement('button');
      themeBtn.className = 'hdr-btn';
      themeBtn.id = 'tlt-hdr-theme-btn';
      themeBtn.textContent = '◑ Theme';
      themeBtn.title = 'Theme-Panel öffnen/schließen';
      themeBtn.addEventListener('click', () => {
        const panel = $('panel-main');
        panel.style.display = panel.style.display === 'none' ? 'flex' : 'none';
      });
      rg.appendChild(themeBtn);

      const helpBtn = document.createElement('button');
      helpBtn.className = 'hdr-btn';
      helpBtn.id = 'tlt-help-chip';
      helpBtn.textContent = '?';
      helpBtn.title = 'Anleitung';
      helpBtn.addEventListener('click', _openHelp);
      rg.appendChild(helpBtn);

      if (window.TUL_PL) {
        const plBtn = document.createElement('button');
        plBtn.className = 'hdr-btn';
        plBtn.textContent = '©';
        plBtn.title = 'Lizenz';
        plBtn.addEventListener('click', window.TUL_PL);
        rg.appendChild(plBtn);
      }

      if (window.TUL_USER) {
        const badge = document.createElement('span');
        badge.className = 'tlt-user-badge';
        badge.title = window.TUL_USER.email || '';
        badge.textContent = (window.TUL_USER.name || '?').slice(0, 2).toUpperCase();
        ...
        logoutBtn.addEventListener('click', () => {
          fetch((_subpath || '') + '/logout', { method: 'POST' })
            .then(() => { location.href = '/'; })
            .catch(() => { location.href = '/'; });
        });
        rg.appendChild(badge);
        rg.appendChild(logoutBtn);
      }
    }
  },
  ...
  _delSnap(i)     { _log.splice(i, 1); renderLog(); saveToServer(getParams()); },
};
