...
def _is_weblog_format(df_raw) -> bool:
    """Format 2 (Web-Download) erkennen: 'SegMode'-Spalte oder 'PS_'-State-Präfix."""
    if 'SegMode' in df_raw.columns:
        return True
    states = df_raw['State'].dropna().astype(str)
    return bool(states.str.startswith('PS_').any())
...
def load_csv(filepath):
    ...
    # Format 2 (Web-Download) normalisieren
    if _is_weblog_format(df):
        df['State'] = df['State'].astype(str).str.replace(r'^PS_', '', regex=True)
        date_fmt = '%d.%m.%Y %H:%M:%S'
        # CRC-Hash aus PrcsInfo entfernen (stört Event-Extraktion nicht, aber sauber)
        if 'PrcsInfo' in df.columns:
            df['PrcsInfo'] = df['PrcsInfo'].astype(str).str.replace(
                r'!<[0-9a-fA-F]+>!', '', regex=True).str.strip()
    else:
        date_fmt = '%d.%m.%y %H:%M:%S'
...
def update_kwh_log(out_dir, csvfile, kwh_val):
    ...
    if d is None:
        try:
            _df = load_csv(csvfile)
            if not _df.empty:
                d = _df['datetime'].iloc[0].date()
        except Exception:
            pass
...
# ══════════════════════════════════════════════════════════════════════════════
# PANEL-MODUS  —  Flask-Web-Interface für tool.milan.how/kurv
# ══════════════════════════════════════════════════════════════════════════════
...
from flask import Flask, Response, request, send_file, jsonify as _jsonify, redirect as _redirect, make_response as _make_response
from werkzeug.middleware.proxy_fix import ProxyFix

_SUBPATH  = _os.environ.get('SUBPATH', '')
_PORT_ENV = int(_os.environ.get('PORT', 5008))
_HERE     = _Path(__file__).parent

_tools_nuc = str(_Path(__file__).resolve().parent.parent)
if _tools_nuc not in _sys.path:
    _sys.path.insert(0, _tools_nuc)
from tul_auth.db import init_db as _init_db, get_conn as _get_conn
from tul_auth.auth import get_current_user as _get_user, is_json_request as _is_json, clear_token_cookie as _clear_token_cookie
from tul_auth.files_routes import make_files_blueprint as _make_files_bp
...
def _tul_auth():
    ...
    if not user:
        ...
        return _redirect('/?next=' + request.path)
    request.tul_user = user

@app.after_request
def _inject_tul_user(resp):
    # Datei-Antworten (inline/download) nicht anfassen – send_file liefert FileWrapper
    if '/files/' in request.path:
        return resp
    if 'text/html' in resp.content_type:
        user = getattr(request, 'tul_user', None)
        snip = f'<script>window.TUL_USER={_json.dumps(user)};</script>'
        body = resp.get_data(as_text=True)
        if '</head>' in body:
            resp.set_data(body.replace('</head>', snip + '\n</head>', 1))
    return resp

_jobs: dict = {}  # jid → {queue, out_dir, done, error}
_TOOL_NAME  = 'kurv'
_FILES_ROOT = _os.environ.get('FILES_ROOT', '/data/tul_files')
app.register_blueprint(_make_files_bp(_TOOL_NAME, _FILES_ROOT))


# ── Kern-Verarbeitung (config-gesteuert, kein input()) ───────────────────────

def run_panel(config: dict, workdir: _Path, out_dir: _Path, log):
    """Batch-Verarbeitung aller CSVs in workdir mit config-Dict statt input()."""
    from datetime import datetime as _dt
    ...
    def _parse_iso(s):
        try: return _date_cls.fromisoformat(s) if s else None
        except ValueError: return None
    _df = _parse_iso(config.get('date_from') or '')
    _dt = _parse_iso(config.get('date_to')   or '')
    csv_files = find_all_csv(workdir, date_from=_df, date_to=_dt)
    if not csv_files:
        log("Keine CSV-Dateien gefunden.")
        return

    log('═' * 52)
    log(f"  Ofen-Log Auswertung  –  {len(csv_files)} Datei(en)")
    log('═' * 52)

    active_ofen_name      = config.get('ofen_name', OFEN_NAME)
    active_ofen_kw        = float(config.get('ofen_kw', OFEN_KW))
    test_threshold        = float(config.get('test_threshold', 20.0))
    window_h              = float(config.get('window_h', WINDOW_HOURS))
    active_theme          = config.get('theme', 'dark')
    out_fmt               = config.get('out_fmt', 'pdf')
    do_einzel_batch       = bool(config.get('einzel_batch', False))
    _at = float(config.get('align_temp', 0) or 0)
    active_align_temp     = _at if _at > 0 else None
    active_align_offset_h = float(config.get('align_offset_h', ALIGN_OFFSET_H) or ALIGN_OFFSET_H)

    kwh_only  = bool(config.get('kwh_only', False))
    thr_str  = f'ab{test_threshold:g}kWh' if test_threshold > 0 else 'alle'
    zr_info  = 'Panel-Upload'
    out_path = out_dir / f'kiln_report_{timestamp}_{thr_str}.pdf'

    # ── kWh-only-Modus: nur Verbrauchslog + Plots, kein Vollbericht ──────────
    if kwh_only:
        log('Nur-kWh-Lauf: berechne Verbrauch …')
        for csvfile in csv_files:
            log(f"► {csvfile.name}")
            try:
                df = load_csv(csvfile)
                kwh = calc_kwh(df, active_ofen_kw) if not df.empty else 0
                if test_threshold > 0 and kwh < test_threshold:
                    log(f"  ⏭ Testbrand ({kwh:.1f} kWh) – übersprungen.")
                else:
                    update_kwh_log(kwh_dir, csvfile, kwh)
                    log(f"  ✓ {kwh:.1f} kWh")
            except Exception as e:
                log(f"  FEHLER: {e}")
        log("\nErstelle kWh-Plots …")
        themes = ['dark', 'light'] if active_theme == 'both' else [active_theme]
        for th in themes:
            build_kwh_plot(kwh_dir, th, active_ofen_name, active_ofen_kw, zr_info, thr_str)
            log(f"  ✓ kWhlog_{th}.pdf")
        return

    jobs = [(f, window_h, TEMP_MAX, None, None) for f in csv_files]

    pages_data      = []
    pages_optimized = []
    clipped_files   = []

    for csvfile, _wh, temp_max, front_h, back_h in jobs:
        log(f"► {csvfile.name}")
        try:
        ...
        if test_threshold > 0 and kwh_pre < test_threshold:
            log(f"  ⏭  Testbrand ({kwh_pre:.1f} kWh < {test_threshold:g} kWh) – übersprungen.")
            try:
        ...
        try:
            ...
            page = process_file(csvfile, _wh, temp_max, front_h, back_h,
                                active_ofen_kw, active_ofen_name,
                                align_temp=active_align_temp,
                                align_offset_h=active_align_offset_h)
            apply_theme(active_theme if active_theme != 'both' else 'dark')
        ...
        if page is None and page_opt is None:
            log("  Keine verwertbaren Daten – übersprungen.")
            log('')
            continue
        ...
        try:
            ...
            if not df_kw.empty:
                update_kwh_log(kwh_dir, csvfile, calc_kwh(df_kw, active_ofen_kw))
        except Exception:
            pass
        log('')

    if not pages_data and not pages_optimized:
        log("Keine Seiten erzeugt.")
        return
    ...
    if out_fmt in ('pdf', 'both'):
        for theme_name in themes_to_render:
            apply_theme(theme_name)
            suffix  = '_(d)' if theme_name == 'dark' else '_(l)'
            pages_d = pages_data
            pages_o = pages_optimized
            if active_theme == 'both':
                pd2, po2 = [], []
                for csvfile, _wh, temp_max, front_h, back_h in jobs:
                    try:
                        ...
                        if test_threshold > 0 and kwh_pre2 < test_threshold:
                            continue
                        apply_theme(theme_name)
                        p  = process_file(csvfile, _wh, temp_max, front_h, back_h,
                                          active_ofen_kw, active_ofen_name,
                                          align_temp=active_align_temp,
                                          align_offset_h=active_align_offset_h)
                        apply_theme(theme_name)
            ...
            if pages_d:
                log(f"Erstelle Standard-PDF [{theme_name}] mit {len(pages_d)} Seite(n) …")
                build_pdf(pages_d, p_std, zr_info, thr_str)
                log(f"✓ Fertig: {p_std.name}")
            if pages_o:
                log(f"Erstelle Optimiert-PDF [{theme_name}] mit {len(pages_o)} Seite(n) …")
                build_pdf(pages_o, p_opt, zr_info, thr_str)
                log(f"✓ Fertig: {p_opt.name}")

    if out_fmt in ('htm', 'both'):
        htm_dir = out_dir / f'htm_log_TC707_{timestamp}'
        htm_dir.mkdir(parents=True, exist_ok=True)
        htm_themes = ['dark', 'light'] if active_theme == 'both' else [active_theme]
            ...
            for csvfile, _wh, temp_max, front_h, back_h in jobs:
                try:
                    ...
                    if active_align_temp is not None:
                        _above = dv.loc[dv['IN0'].notna() & (dv['IN0'] >= active_align_temp), 'datetime']
                        if not _above.empty:
                            t_start = _above.iloc[0] - timedelta(hours=active_align_offset_h)
                    dur_min   = int(round((dv['datetime'].iloc[-1] - t_start).total_seconds() / 60))
                    valid     = dv['IN0'].notna()
                    max_t     = dv.loc[valid, 'IN0'].max() if valid.any() else 0
                    kwh       = calc_kwh(dv, active_ofen_kw)
                    title_opt = (f"{active_ofen_name}  ·  {csvfile.stem}   │   "
                                 f"T-max: {max_t:.0f} °C   │   "
                                 f"Dauer: {dur_min//60:02d}:{dur_min%60:02d} h   │   "
                                 f"∼ {kwh:.1f} kWh")
                    info_opt  = (f"Start: {t_start.strftime('%d.%m.%Y %H:%M')}  │  "
                                 f"Ende: {dv['datetime'].iloc[-1].strftime('%d.%m.%Y %H:%M')}  │  "
                                 f"Nennleistung: {active_ofen_kw:.1f} kW")
                    fig_opt   = draw_plot_plotly(dv, title_opt, info_opt, theme_name)

                    t_win_end = t_start + __import__('datetime').timedelta(hours=_wh)
                    mask      = (df['datetime'] >= t_start) & (df['datetime'] <= t_win_end)
                    ...
                    info_std  = (f"Start: {t_start.strftime('%d.%m.%Y %H:%M')}  │  "
                                 f"Fenster: {_wh:.0f} h / {temp_max:.0f} °C")
                    fig_std   = draw_plot_plotly(
                        dv_std if len(dv_std) > 1 else dv, title_std, info_std, theme_name,
                        x_end=t_win_end)

                    figs     = [(fig_opt, "Optimiert"), (fig_std, f"Standard ({_wh:.0f}h)")]
                    htm_path = htm_dir / f"ausw_{csvfile.stem}_{timestamp}{suffix}.html"
                    htm_events = []
                    ...
                    log(f"Erstelle HTML [{theme_name}]: {htm_path.name} …")
                    build_html(figs, htm_path, theme_name, events=htm_events or None)
                    log(f"✓ Fertig: {htm_path.name}")
                except Exception as e:
                    import traceback
                    log(f"  HTML FEHLER {csvfile.name}: {e}")
                    log(traceback.format_exc())
        ...
        for th in ['dark', 'light']:
            ...
            if out_fmt in ('pdf', 'both'):
                ...
                if kwh_matches:
                    ...
                    for pdf_path in sorted(out_dir.glob(f'*{suffix_th}*.pdf')):
                        try:
                            append_pdf(pdf_path, kwh_pdf)
                            log(f"  + kWhlog → {pdf_path.name}")
                        except Exception as e:
                            log(f"  ⚠ kWhlog-Anhang fehlgeschlagen ({pdf_path.name}): {e}")
    except Exception as e:
        log(f"  kWh-Plot Fehler: {e}")

    if clipped_files:
        log(f"\n⚠  Abgeschnitten (Daten über {window_h:.0f}-h-Fenster hinaus):")
        for fn in clipped_files:
            log(f"     • {fn}")

    if do_einzel_batch and len(jobs) > 1:
        ...
        for csvfile, _wh, temp_max, front_h, back_h in jobs:
            ...
            for theme_name in themes_to_render:
                try:
                    apply_theme(theme_name)
                    pe  = process_file(csvfile, _wh, temp_max, front_h, back_h,
                                       active_ofen_kw, active_ofen_name)
                    apply_theme(theme_name)
                    peo = process_file_optimized(csvfile, active_ofen_kw, active_ofen_name)
                except Exception as e:
                    log(f"  FEHLER {csvfile.name}: {e}")
                    continue
                combined = []
                if pe:
                ...
                if combined:
                    ...
                    log(f"  → {einzel_path.name}")
                    build_pdf(combined, einzel_path, zr_info, thr_str)
        log("✓ Einzelversionen fertig.")
...
def index():
    ...
    return _R(html, mimetype='text/html', headers={'Cache-Control': 'no-store'})
...
def theme_post():
    ...
    uid = request.tul_user['id']
    incoming = _json2.loads(request.get_data(as_text=True) or '{}')
    with _get_conn() as conn:
        row = conn.execute(
            'SELECT settings FROM user_themes WHERE user_id=? AND tool=?', (uid, _TOOL_NAME)
        ).fetchone()
        existing = _json2.loads(row['settings']) if row else {}
        existing.update(incoming)  # Theme-Keys überschreiben, presets bleiben erhalten
        conn.execute(
            'INSERT INTO user_themes(user_id,tool,settings) VALUES(?,?,?) '
            'ON CONFLICT(user_id,tool) DO UPDATE SET settings=excluded.settings',
            (uid, _TOOL_NAME, _json2.dumps(existing))
        )
    return ('', 204)


def _pool_dir() -> _Path:
    d = _Path(_FILES_ROOT) / 'input'
...
def upload():
    """Einzelner CSV-Upload in den Pool (für Einzel-Analysen)."""
    import mimetypes as _mt2
    files   = request.files.getlist('files')
    user_id = request.tul_user['id']
    if not files:
        return {'ok': False, 'error': 'Keine Dateien'}, 400
    pool = _pool_dir()
    from tul_auth.db import uid as _uid2, now as _now2
    saved = []
    ts = _now2()
    with _get_conn() as conn:
        for f in files:
            if not (f.filename and f.filename.lower().endswith('.csv')):
                continue
            name = _Path(f.filename).name
            dest = pool / name
            f.save(str(dest))
            size = dest.stat().st_size
            ex = conn.execute(
                'SELECT id FROM files WHERE filename=? AND tool=? AND category=? AND grp IS NULL',
                (name, _TOOL_NAME, 'input')
            ).fetchone()
            if ex:
                conn.execute('UPDATE files SET size=?, created_at=? WHERE id=?',
                             (size, ts, ex['id']))
            else:
                conn.execute(
                    'INSERT INTO files'
                    ...
                    ' VALUES(?,?,?,?,?,?,?,?,?,?,?,?)',
                    (_uid2(), user_id, _TOOL_NAME, name, str(dest), size,
                     'text/csv', 'input', 'burn-log', '1mo', ts, None)
                )
            saved.append(name)
    if not saved:
        return {'ok': False, 'error': 'Keine CSV-Dateien im Upload.'}, 400
    return {'ok': True, 'n': len(saved), 'files': saved}
...
def run_job():
    ...
    # Auswahl: alle Pool-CSVs oder nur explizit gewählte
    sel = cfg.pop('selected_files', None)  # Liste von Dateinamen oder None = alle
    if sel:
        csv_paths = [pool / n for n in sel if (pool / n).exists() and n.lower().endswith('.csv')]
    else:
        csv_paths = sorted(pool.glob('*.csv'))

    if not csv_paths:
        return {'ok': False, 'error': 'Keine CSV-Dateien im Pool.'}, 400

    jid     = _uuid.uuid4().hex
    out_dir = _Path(_tempfile.mkdtemp(prefix=f'kurv_{jid}_out_'))
    q: _queue.Queue = _queue.Queue()
    _jobs[jid] = {'out_dir': out_dir, 'queue': q, 'done': False, 'error': None}

    def worker():
        def log(msg):
            q.put(str(msg))
        try:
            import shutil as _shutil, mimetypes as _mt2
            from tul_auth.db import uid as _uid2, now as _now2
            from datetime import datetime as _dt2

            log(f'Worker gestartet – {len(csv_paths)} CSV-Datei(en)')

            # run_panel erwartet ein Verzeichnis; wir bauen ein tmpdir mit Symlinks
            wdir = _Path(_tempfile.mkdtemp(prefix=f'kurv_{jid}_sel_'))
            seen_names: set = set()
            for p in csv_paths:
                if p.name in seen_names:
                    continue
                seen_names.add(p.name)
                (wdir / p.name).symlink_to(p)

            log(f'Starte run_panel …')
            run_panel(cfg, wdir, out_dir, log)
            log('run_panel fertig – registriere Ausgaben …')

            # Outputs persistent speichern und in DB registrieren
            job_label = _dt2.now().strftime('%Y%m%d_%H%M') + '_' + jid[:8]
            out_persist = _Path(_FILES_ROOT) / 'output' / 'kurv' / job_label
            ...
            with _get_conn() as conn:
                for src in out_dir.rglob('*'):
                    ...
                    mime = _mt2.guess_type(src.name)[0] or 'application/octet-stream'
                    suf = src.suffix.lower()
                    if suf == '.html':
                        ft = 'burn-report-html'
                    elif src.name == 'kWh.log':
                        ft = 'kwh-log'
                    elif src.name.lower().startswith('kwhlog') and suf == '.pdf':
                        ft = 'burn-report-kwh'
                    else:
                        ft = 'burn-report'
                    conn.execute(
                        ...
                        '(id,user_id,tool,filename,path,size,mime,category,file_type,retention,created_at,grp)'
                        ' VALUES(?,?,?,?,?,?,?,?,?,?,?,?)',
                        (_uid2(), user_id, _TOOL_NAME, src.name, str(dst),
                         dst.stat().st_size, mime, 'output', ft, 'user', ts, job_label)
                    )
            _jobs[jid]['grp'] = job_label
            log(f'✓ Ausgabe in DV: {job_label}')
        except Exception as e:
            import traceback
...
def presets_get():
    ...
    data = _json2.loads(row['settings']) if row else {}
    return _jsonify(data.get('presets', []))
...
def presets_post():
    ...
    if not name or config is None:
        return _jsonify({'error': 'name und config erforderlich'}), 400
    with _get_conn() as conn:
        row = conn.execute(
            'SELECT settings FROM user_themes WHERE user_id=? AND tool=?',
            (uid_val, _TOOL_NAME)
        ...
        presets = [p for p in presets if p.get('name') != name]  # upsert
        presets.append({'name': name, 'config': config})
        data['presets'] = presets
        conn.execute(
            'INSERT INTO user_themes(user_id,tool,settings) VALUES(?,?,?)'
            ' ON CONFLICT(user_id,tool) DO UPDATE SET settings=excluded.settings',
            (uid_val, _TOOL_NAME, _json2.dumps(data))
        )
...
def download(jid):
    ...
    if not out_files:
        return 'Keine Ausgabedateien', 404

    if len(out_files) == 1:
        f = out_files[0]
        return send_file(str(f), as_attachment=True, download_name=f.name)

    zip_path = out_dir.parent / f'kurv_{jid}.zip'
...
def logout():
    return _clear_token_cookie(_make_response(_jsonify({'ok': True})))


# ── Hauptprogramm ────────────────────────────────────────────────────────────

def main():
    parser = argparse.ArgumentParser(description='Ofen-Log CSV → PDF (Panel/CLI)')
    parser.add_argument('--workdir', default=None)
    parser.add_argument('--panel',   action='store_true', help='Flask-Panel starten')
    args = parser.parse_args()

    if args.panel or _os.environ.get('DOCKER'):
        print(f"Kurv-Panel startet auf Port {_PORT_ENV} …")
        app.run(host='0.0.0.0', port=_PORT_ENV, debug=False, threaded=True)
    else:
        workdir = Path(args.workdir) if args.workdir else Path.cwd()
        from datetime import datetime as _dt2
        timestamp = _dt2.now().strftime('%Y%m%d_%H%M')
        out_dir   = workdir / f'pdf_log_TC707_{timestamp}'
        out_dir.mkdir(parents=True, exist_ok=True)
        run_panel(
            config={'ofen_name': OFEN_NAME, 'ofen_kw': OFEN_KW,
                    'test_threshold': 20.0, 'window_h': WINDOW_HOURS,
                    'theme': THEME, 'out_fmt': 'pdf'},
            workdir=workdir,
            out_dir=out_dir,
            log=print,
        )
